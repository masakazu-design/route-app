...
# API設定（Streamlit Secretsから取得、ローカル開発時はデフォルト値）
try:
...
def load_schedules():
    ...
    if not supabase:
        return [], "Supabase未設定"
    try:
        result = supabase.table("route_schedules").select("*").order("created_at", desc=True).limit(20).execute()
        return result.data, None
    except Exception as e:
        return [], str(e)

def load_schedule_by_id(schedule_id):
    """IDでスケジュールを取得"""
    supabase = get_supabase_client()
    if not supabase:
        return None, "Supabase未設定"
    try:
        result = supabase.table("route_schedules").select("*").eq("id", schedule_id).execute()
        if result.data:
...
def delete_schedule(schedule_id):
    ...
    try:
        supabase.table("route_schedules").delete().eq("id", schedule_id).execute()
        return True, None
    except Exception as e:
        return False, str(e)

def save_history(execution_date, schedule_id=None, notes="", status="completed"):
    """実行履歴を保存"""
    supabase = get_supabase_client()
    if not supabase:
        return None, "Supabase未設定"
...
def load_history():
    ...
    if not supabase:
        return [], "Supabase未設定"
    try:
        result = supabase.table("route_history").select("*, route_schedules(name, num_days)").order("execution_date", desc=True).limit(50).execute()
        return result.data, None
    except Exception as e:
        return [], str(e)
...
# 正規化後のターゲットレイヤー（O/0の両方に対応）
TARGET_LAYERS_NORMALIZED = ["施工中工事", "O2グループ", "02グループ", "発注先"]
...
VISIT_FORBIDDEN_START_HOUR = 11
VISIT_FORBIDDEN_START_MINUTE = 30
        ...
        for orig_idx, origin_chunk in enumerate(origin_chunks):
            for dest_idx, dest_chunk in enumerate(dest_chunks):
                ...
                if progress_callback:
                    ...
                    bias_info = f"（冬季モード）" if datetime.now().month in WINTER_MONTHS else ""
                    progress_callback(progress, f"距離行列取得中{bias_info}... ({current_request}/{total_requests})")
...
def global_tsp_time_slice_allocation(
    ...
    # ============================================
    # Step 3: きたえるーむは後で配置（Time Slicing後に最適な日に追加）
    # ============================================
    # きたえるーむはTime Slicing後に、17:00に訪問可能な日に配置する
    ...
    # ============================================
    # Step 4: Time Slicing（時間シミュレーションで日程分割）
    # ============================================
    ...
    if kitaeroom_indices:
        ...
        if best_day >= 0:
            day_routes[best_day].extend(kitaeroom_indices)
        else:
            # 訪問がある最後の日を探す
            for day_idx in range(num_days - 1, -1, -1):
                if day_routes[day_idx]:
                    day_routes[day_idx].extend(kitaeroom_indices)
                    break
            else:
                # どの日も訪問がない場合は1日目に追加
                day_routes[0].extend(kitaeroom_indices)
...
def create_day_timetable(day_num, visit_indices, visit_df, time_matrix_all,
    ...
    for i, visit_idx in enumerate(filtered_visit_indices):
        ...
        if should_insert_lunch:
            ...
            if actual_lunch_duration >= 30:
                ...
                # 表示名を決定
                if move_time_min > 0:
                    restaurant_name = f"（60分）昼食休憩【移動：{move_time_min}分】"
                else:
                    restaurant_name = f"（60分）昼食休憩"
                    if api_key:
    ...
    if not lunch_inserted and shacho_return_arrival >= lunch_check_time:
        ...
        if actual_lunch_duration >= 30:
            ...
            # 表示名を決定
            if last_to_shacho_min > 0:
                restaurant_name = f"（60分）昼食休憩【移動：{last_to_shacho_min}分】"
            else:
                restaurant_name = f"（60分）昼食休憩"
                if api_key:
...
# 移動時間バイアス表示
st.sidebar.subheader("🚗 移動時間設定")
current_month = datetime.now().month
is_winter = current_month in WINTER_MONTHS
bias = get_travel_time_bias()
bias_percent = int((bias - 1) * 100)

if is_winter:
    st.sidebar.warning(f"❄️ **冬季モード適用中**\n移動時間 +{bias_percent}%（12月〜3月）")
else:
    st.sidebar.info(f"☀️ **通常モード**\n移動時間 +{int((TRAVEL_TIME_BIAS_BASE - 1) * 100)}%")
...
if supabase_client:
    ...
    # スケジュール読み込み
    st.sidebar.markdown("**📂 スケジュールを読み込み**")
    schedules, err = load_schedules()
    if schedules:
    ...
    # 履歴
    with st.sidebar.expander("📅 実行履歴", expanded=False):
        history, err = load_history()
        if history:
            for h in history[:5]:
                sch_info = h.get('route_schedules', {})
                sch_name = sch_info.get('name', '不明') if sch_info else '不明'
                st.markdown(f"**{h['execution_date']}** - {sch_name}")
                if h.get('actual_notes'):
                    st.caption(h['actual_notes'])
        else:
            st.info("履歴がありません")

        st.markdown("---")
        exec_date = st.date_input("実行日", key="history_date")
        exec_notes = st.text_input("メモ", placeholder="変更点など", key="history_notes")
        if st.button("📝 記録", key="btn_save_history", use_container_width=True):
    ...
    st.sidebar.caption("secrets.tomlにSUPABASE_URLとSUPABASE_KEYを設定")
...
if map_df is not None and len(map_df) > 0:
    ...
    for target in TARGET_LAYERS_NORMALIZED:
        ...
        # レイヤーアイコン設定（O2/02両方に対応）
        if "O2" in target or "02" in target or "グループ" in target:
            layer_icon = "🏢"
